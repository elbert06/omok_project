...
omok_data = ""
turn = True
while os.path.isfile("drive/MyDrive/train2/"+str(file_name)+".txt"):
    # y_test = game_init()
    if(file_name % 10 == 0):
        print(str(file_name))
    x_test = game_init()
    ...
    x_test = game_init()
    try:
      notepad = open("drive/MyDrive/train2/"+str(file_name)+".txt","r")
    except:
      continue
    turn = True
...
model = Sequential()
model.add(Conv2D(64, kernel_size=4, padding='same', activation='relu',input_shape=(15,15,1)))
...
xt = np.array(x)
yt = np.array(y)
model.fit(xt,yt,epochs=100,verbose=2)
