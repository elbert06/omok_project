...
def omok():
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LEFT:
                    ...
                    winning,decide = key.who(decide,gameboard_list,winner_row,winner_column,winning)
                ...
                if x < 15 and y < 15:
                    book_x = 50 * x + 35
                    book_y = 50 * y + 35
        end = int(timeit.default_timer())
        if decide == True:
            test = pygame.mixer.Sound(current+"/picture/get.mp3")
            test.play(0)
        elif turn == False:
            if int(end - start) != turn_last:
                turn_last = int(end-start)
                black_second += 1
        elif turn == True:
            if int(end - start) != turn_last:
                turn_last = int(end-start)
        ...
        screen.blit(black_time, ((screen_wid / 8 * 7),(screen_hei / 3 * 1)))
        screen.blit(white_time, ((screen_wid / 8 * 7),(screen_hei / 3 * 2)))
